...
def get_current_url(
    ...
    cat(url_quote(wsgi_get_bytes(environ.get("JS_KEY", ""))).rstrip("/"))
    cat("/")
    if not root_only:
...
def get_script_name(environ, charset="utf-8", errors="replace"):
    """Returns the `JS_KEY` from the WSGI environment and properly
    decodes it.  This also takes care about the WSGI decoding dance
    on Python 3 environments.  if the `charset` is set to `None` a
    bytestring is returned.
    ...
    :param errors: the decoding error handling.
    """
    path = wsgi_get_bytes(environ.get("JS_KEY", ""))
    return to_unicode(path, charset, errors, allow_none_charset=True)


def pop_path_info(environ, charset="utf-8", errors="replace"):
    """Removes and returns the next segment of `PATH_INFO`, pushing it onto
    `JS_KEY`.  Returns `None` if there is nothing left on `PATH_INFO`.

    If the `charset` is set to `None` a bytestring is returned.

    If there are empty segments (``'/foo//bar``) these are ignored but
    properly pushed to the `JS_KEY`:

    >>> env = {'JS_KEY': '/foo', 'PATH_INFO': '/a/b'}
    >>> pop_path_info(env)
    'a'
    >>> env['JS_KEY']
    '/foo/a'
    ...
    script_name = environ.get("JS_KEY", "")
    ...
    if "/" not in path:
        environ["PATH_INFO"] = ""
        environ["JS_KEY"] = script_name + path
        rv = wsgi_get_bytes(path)
    else:
        segment, path = path.split("/", 1)
        environ["PATH_INFO"] = "/" + path
        environ["JS_KEY"] = script_name + segment
